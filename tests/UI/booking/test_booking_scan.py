...
class TestBookingScan(BaseTest):
    ...
    def test_booking_scan(self, page: Page, test_card_info: dict) -> None:
        ...
        select_plan_page = (
            SelectPlanPage.for_returning_user(page)
            if is_returning
            else SelectPlanPage.for_new_user(page)
        )
        ...
        schedule_scan_page = (
            ScheduleScanPage.for_returning_user(page)
            if is_returning
            else ScheduleScanPage.for_new_user(page)
        )
        schedule_scan_page.verify_url()
        schedule_scan_page.verify_page_elements()
        schedule_scan_page.verify_scheduling_scan()

        reserve_appointment_page = (
            ReserveAppointmentPage.for_returning_user(page)
            if is_returning
            else ReserveAppointmentPage.for_new_user(page)
        )
        ...
        scan_confirm_page = (
            ScanConfirmPage.for_returning_user(page)
            if is_returning
            else ScanConfirmPage.for_new_user(page)
        )
