...
class ScheduleScanPage(BasePage):
    ...
    RETURNING_PATH = "/book-scan/schedule-scan"

    @classmethod
    def for_new_user(cls, page: Page) -> "ScheduleScanPage":
        return cls(page, path=cls.PATH)

    @classmethod
    def for_returning_user(cls, page: Page) -> "ScheduleScanPage":
    ...
    def __init__(self, page: Page, path: str = PATH):
        ...
        self.state_dropdown_selector = Dropdown(
            self.page, "div[class='multiselect']", options=self.state_dropdown_options
        )
        ...
        self.calendar_open_date_selector = (
            "div[class*='vuecal__cell--day']:not([class*='disabled'])"
        )
        self.time_slot_selector = (
            "div[class*='individual-appointment']:not([style='display: none;'])"
        )
        self.continue_button_selector = (
            "button[data-test='submit']:has-text('Continue')"
        )
    ...
    def verify_page_elements(self) -> None:
        ...
        expect(
            self.page.locator(self.locator_card_selector).first
        ).to_be_visible()  # Would get data from DB to get exact number of locator cards
