...
class Dropdown(BaseComponent):
    ...
    def select_option(self, option_text: str) -> None:
        if option_text not in self.options:
            raise ValueError(f"Option '{option_text}' is not a valid option. Valid options are: {self.options}")
    ...
    def verify_select_options(self) -> None:
        ...
        for option in self.options:
            assert option in texts, f"Expected option '{option}' not found. Found: {texts}"
